...
<details>
  ...
  > [!WARNING]  
  > At Remove packages step, the script will prompt for your confirmation before proceeding. Once confirmed, it will **remove all packages** associated with the Caelestia setup. This includes any packages that were already pre-installed on your system but happen to match the setup requirements. Please review the **Software Stack** section above for the comprehensive list of packages that will be uninstalled.
