...
ui_section "Step 6/11 - Services and KWin"
...
if [[ -f "$HOME/.config/systemd/user/qs-kwin-bridge.service" ]] && \
   [[ -s "$HOME/.config/systemd/user/qs-kwin-bridge.service" ]]; then
    ui_info "Enabling qs-kwin-bridge service..."
    systemctl --user daemon-reload
    systemctl --user enable --now qs-kwin-bridge.service 2>/dev/null || true
    ui_ok "qs-kwin-bridge enabled."
else
    ui_skip "qs-kwin-bridge.service is empty or missing."
fi
...
ui_info "Setting up ydotoold (OSK key injection daemon)..."
...
if [[ ! -f /etc/udev/rules.d/80-uinput.rules ]]; then
    ...
    ui_ok "udev rule for uinput created."
fi

# Add user to 'input' group (takes effect on next login)
if ! groups "$USER" | grep -q '\binput\b'; then
    sudo usermod -aG input "$USER"
    ui_ok "Added $USER to input group (takes effect on next login)."
else
    ui_ok "$USER is already in the input group."
fi
...
if [[ ! -f "$SUDOERS_FILE" ]]; then
    ...
    ui_ok "sudoers NOPASSWD rule added for ydotoold."
else
    # Update it with the current username
    echo "$USER ALL=(root) NOPASSWD: /usr/bin/ydotoold" | sudo tee "$SUDOERS_FILE" > /dev/null
    sudo chmod 440 "$SUDOERS_FILE"
    ui_ok "sudoers NOPASSWD rule updated for $USER."
fi
...
ui_ok "ydotoold-wrapper deployed to ~/.local/bin."
...
if [[ -f "${BUNDLE_DIR:-$(dirname "$(dirname "$0")")}/src/systemd/ydotoold.service" ]]; then
    ...
    systemctl --user start ydotoold.service 2>/dev/null || \
        ui_info "ydotoold will start on next login."
    ui_ok "ydotoold service configured."
fi

# Reload KWin and KGlobalAccel
ui_info "Reloading KWin..."
qdbus6 org.kde.KWin /KWin reconfigure 2>/dev/null || true

ui_info "Restarting plasma-kglobalaccel..."
systemctl --user restart plasma-kglobalaccel.service 2>/dev/null || true

ui_ok "Services configured."
