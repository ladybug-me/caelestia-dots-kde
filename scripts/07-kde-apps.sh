...
if [[ "${APPLY_MATERIAL_YOU:-true}" == "true" ]]; then
    if [[ "$BASE_DISTRO" == "arch" ]]; then
        ...
        if ! command -v kde-material-you-colors >/dev/null 2>&1; then
            ui_info "Installing kde-material-you-colors via uv..."
            sudo dnf install -y dbus-devel dbus-glib-devel python3-devel
            uv tool install kde-material-you-colors >/dev/null 2>&1 || {
                ui_warn "Could not install kde-material-you-colors. Skipping."
            }
        else
            ui_skip "kde-material-you-colors is already installed."
        fi
    fi
else
    ui_skip "Skipping kde-material-you-colors installation. Uninstalling if present..."
...
# Update plasma configuration for default look/feel if needed
    ...
    ui_ok "KDE extra apps step complete."
