#!/usr/bin/env bash
# 09-system-tweaks.sh - Apply live system configuration tweaks to the running KDE session.
# This script only writes config values and reloads KDE daemons.
# It does not copy files.
# Usage:
#   bash scripts/09-system-tweaks.sh
#   bash scripts/09-system-tweaks.sh --list

set -uo pipefail

BUNDLE_DIR="${BUNDLE_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
source "$BUNDLE_DIR/scripts/00-ui.sh"

ui_section "Step 9/11 - Live System Tweaks"

# Disable KDE OSD popups.
tweak_disable_kde_osd() {
    ui_info "Disabling KDE OSD popups (volume and brightness)..."

    kwriteconfig6 --file plasmarc --group "OSD" --key "Enabled" "false" 2>/dev/null || true
    kwriteconfig6 --file plasmarc --group "OSD" --key "ShowOnActiveScreen" "false" 2>/dev/null || true
    kwriteconfig6 --file kdeglobals --group "KDE" --key "OSDEnabled" "false" 2>/dev/null || true
    kwriteconfig6 --file plasmanotifyrc --group "Notifications" --key "LoudnessChangedOSD" "false" 2>/dev/null || true
    kwriteconfig6 --file powerdevilrc --group "BrightnessControl" --key "showOSD" "false" 2>/dev/null || true
    kwriteconfig6 --file powerdevilrc --group "AC" --key "brightnessosd" "false" 2>/dev/null || true

    mkdir -p "$HOME/.config"
    if [[ -f "$HOME/.config/kmixrc" ]]; then
        sed -i 's/^ShowOSD=.*/ShowOSD=false/' "$HOME/.config/kmixrc" 2>/dev/null || true
        grep -q "^ShowOSD=" "$HOME/.config/kmixrc" || echo -e "\n[Global]\nShowOSD=false" >> "$HOME/.config/kmixrc"
    else
        cat > "$HOME/.config/kmixrc" <<'EOF'
[Global]
ShowOSD=false
EOF
    fi

    ui_ok "KDE OSD popups disabled."
}

# Create five virtual desktops.
tweak_five_desktops() {
    ui_info "Configuring 5 virtual desktops..."

    kwriteconfig6 --file kwinrc --group "Desktops" --key "Number" "5"
    kwriteconfig6 --file kwinrc --group "Desktops" --key "Rows" "1"
    for i in $(seq 1 5); do
        kwriteconfig6 --file kwinrc --group "Desktops" --key "Name_$i" "Desktop $i"
    done

    ui_ok "5 virtual desktops configured."
}

# Register workspace switching shortcuts.
tweak_workspace_shortcuts() {
    ui_info "Registering Meta+1..5 workspace switching shortcuts..."

    for i in $(seq 1 5); do
        kwriteconfig6 --file kglobalshortcutsrc --group "kwin" --key "Switch to Desktop $i" "Meta+$i,none,Switch to Desktop $i"
    done

    for i in $(seq 1 5); do
        kwriteconfig6 --file kglobalshortcutsrc --group "kwin" --key "Window to Desktop $i" "Meta+Shift+$i,none,Move Window to Desktop $i"
    done

    ui_ok "Workspace shortcuts registered."
}

# Reload KDE daemons.
tweak_reload_kde() {
    ui_info "Reloading KWin and plasma-kglobalaccel..."
    qdbus6 org.kde.KWin /KWin reconfigure 2>/dev/null || true
    systemctl --user restart plasma-kglobalaccel.service 2>/dev/null || true
    kbuildsycoca6 --noincremental 2>/dev/null || true
    ui_ok "KDE daemons reloaded."
}

# Set default shell to Fish.
tweak_default_shell() {
    ui_info "Setting default shell to fish..."

    if command -v fish >/dev/null 2>&1; then
        local fish_path
        fish_path="$(command -v fish)"

        printf '%s\n' "${SUDO_PASS:-}" | sudo -S chsh -s "$fish_path" "$USER" 2>/dev/null || ui_warn "Failed to change shell for $USER. You may need to run 'chsh -s $fish_path' manually."

        local konsole_profile_dir="$HOME/.local/share/konsole"
        mkdir -p "$konsole_profile_dir"

        local profiles_found=0
        for profile in "$konsole_profile_dir"/*.profile; do
            if [[ -f "$profile" ]]; then
                kwriteconfig6 --file "$profile" --group "General" --key "Command" "$fish_path"
                profiles_found=1
            fi
        done

        if [[ $profiles_found -eq 0 ]]; then
            kwriteconfig6 --file "$konsole_profile_dir/Profile 1.profile" --group "General" --key "Name" "Profile 1"
            kwriteconfig6 --file "$konsole_profile_dir/Profile 1.profile" --group "General" --key "Command" "$fish_path"
            kwriteconfig6 --file "$HOME/.config/konsolerc" --group "Desktop Entry" --key "DefaultProfile" "Profile 1.profile"
        fi
    else
        ui_warn "Fish is not installed, skipping shell change."
    fi

    ui_ok "Shell configuration applied."
}

# Patch caelestia CLI to avoid terminal sequence bleeding.
tweak_patch_caelestia_cli() {
    ui_info "Patching caelestia CLI to fix terminal sequence bleeding..."

    local theme_file
    theme_file=$(python3 -c "import importlib.util; spec = importlib.util.find_spec('caelestia.utils.theme'); print(spec.origin) if spec and spec.origin else print('')" 2>/dev/null)

    if [[ -n "$theme_file" && -f "$theme_file" ]]; then
        local python_code="
import sys, pathlib, subprocess, re
p = pathlib.Path('$theme_file')
text = p.read_text()
old = '''    for pt in pts_path.iterdir():
        if pt.name.isdigit():
            try:
                # Use non-blocking write with timeout to prevent hangs'''
new = '''    for pt in pts_path.iterdir():
        if pt.name.isdigit():
            try:
                res = subprocess.run([\"ps\", \"-t\", pt.name, \"-o\", \"comm=\"], capture_output=True, text=True)
                processes = [p.strip() for p in res.stdout.splitlines() if p.strip()]
                if not any(re.match(r\"^(bash|zsh|fish|sh|dash|mksh|tcsh|csh|ksh)$\", p) for p in processes):
                    continue
            except Exception:
                pass
            try:
                # Use non-blocking write with timeout to prevent hangs'''
if old in text:
    p.write_text(text.replace(old, new))
"
        if ! python3 -c "$python_code" 2>/dev/null; then
            if ! printf '%s\n' "${SUDO_PASS:-}" | sudo -S python3 -c "$python_code" 2>/dev/null; then
                ui_warn "Failed to patch $theme_file (requires sudo)"
                echo "Caelestia CLI Theme Sequence Patch" >> "${XDG_CACHE_HOME:-$HOME/.cache}/caelestia-kde/failed_patches.txt"
            fi
        fi
        ui_ok "caelestia CLI patched."
    else
        ui_warn "caelestia CLI not found, skipping patch."
    fi
}

if [[ "${1:-}" == "--list" ]]; then
    echo
    echo "Available tweaks:"
    declare -F | awk '/^declare -f tweak_/ {print "  -", substr($3, 7)}' | sed 's/_/ /g'
    echo
    exit 0
fi

tweak_disable_kde_osd
tweak_five_desktops
tweak_workspace_shortcuts
tweak_default_shell
tweak_patch_caelestia_cli
tweak_reload_kde

ui_ok "All system tweaks applied successfully."
echo
