...
CustomMouseArea {
    ...
    onPressed: event => {
        ...
        if (root.focusGrab && (root.focusGrab.active || popouts.isDetached)) {
            ...
            if (event.x < bar.clampedWidth) inside = true;
            ...
            else if (visibilities.session && inRightPanel(panels.sessionWrapper, event.x, event.y) && withinPanelHeight(panels.sessionWrapper, event.x, event.y)) inside = true;
            else if (visibilities.sidebar && event.x > Math.min(width - Config.border.minThickness, bar.implicitWidth + panels.sidebar.x)) inside = true;
            ...
            else if (popouts.hasCurrent && inLeftPanel(panels.popoutsWrapper, event.x, event.y) && withinPanelHeight(panels.popoutsWrapper, event.x, event.y)) inside = true;
